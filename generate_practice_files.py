...
def generate_day05():
    ...
    add_instruction_sheet(wb, 'Day 5 练习：数据验证与条件格式', [
        ...
        '',
        '【数据验证】',
        '1. 选中"分类"列→数据→数据验证→序列→来源输入"电子产品,服装鞋帽,食品饮料,家居用品,运动户外"',
        '   → 现在分类列只能从下拉列表里选，不能随便打字了',
        '2. 选中"价格"列→数据验证→小数→介于13到3000→防止输入异常价格',
        '3. 选中"库存"列→数据验证→整数→介于0到9999',
        '',
        '【条件格式】',
...
def generate_day10():
    ...
    add_instruction_sheet(wb, 'Day 10 练习：VLOOKUP', [
        ...
        '',
        '【注意】VLOOKUP只能从左往右查。如果想"根据姓名查ID"（从右往左），需要用INDEX+MATCH或XLOOKUP。',
    ])
