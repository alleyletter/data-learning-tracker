...
def generate_day05():
    ...
    add_instruction_sheet(wb, 'Day 5 练习：数据验证与条件格式', [
        ...
        '【备用实战】"靓号定价(备用实战)"工作表模拟靓号定价表（与真实后台列一致），练数据验证和条件格式用：',
        '  1. 价格列→条件格式→数据条（条越长价格越高）',
        '  2. 位数列→4条"等于"规则：5=红/6=橙/7=黄/8=绿（条件格式→突出显示→等于，逐条设置；色阶是渐变，给不了指定颜色）',
        '  3. 靓号ID列→条件格式→突出显示→文本包含→888→自定义格式→黄色填充',
        '',
    ...
    add_data_sheet(wb, '靓号定价(备用实战)', ['靓号ID', '位数', '类别', '价格(乐元)', '购买方式', '赠送VIP'],
                   [[r[0], r[1], r[2], r[3], r[4], r[5]] for r in generate_haoid_rows(60)])
